...
class BWGViewAlbum_compact_preview {
  ...
  public function display($params, $from_shortcode = 0, $bwg = 0) {
    ...
    if ($type == 'gallery') {
      ...
      $image_rows = $this->model->get_image_rows_data($album_gallery_id, $items_per_page, 'filename', $bwg, $sort_direction);
      $images_count = count($image_rows);
     print_r($params['sort_by']);
	 if (!$image_rows) {
...
<style>
.justinsGalGroup{float: left; width: 15%;}
.justinsGal{}
.justinsGal a{display: block; font-size: 14px!important; height: 40px; line-height: 40px; width: 100px; color: #000; text-decoration: none;  outline: 0!important;}
.justinsGal a.active{border-bottom: 2px solid #444;}

#bwg_container1_0{width: 85%; float: left;}
.bwg_back_0{display: none!important;}

.entry-content{max-width: 100%!important;}

.bwg_standart_thumbnails_0 {display:block!important;}
.bwg_standart_thumbnails_0:after {
  content: '';
  display: block;
  clear: both;
}

.bwg_standart_thumbnails_0 .bwg_lightbox_0 .bwg_standart_thumb_0{display: block; }
.bwg_standart_thumbnails_0 .bwg_lightbox_0 .bwg_standart_thumb_0 .bwg_standart_thumb_spun1_0{display: block; width: 100%!important; height: 100%!important;}
.bwg_standart_thumbnails_0 .bwg_lightbox_0 .bwg_standart_thumb_0 .bwg_standart_thumb_spun1_0 .bwg_standart_thumb_spun2_0{display: block; width: 100%!important; height: 100%!important;}
.bwg_standart_thumbnails_0 .bwg_lightbox_0 .bwg_standart_thumb_0 .bwg_standart_thumb_spun1_0 .bwg_standart_thumb_spun2_0 img,
.bwg_img_custom {display: block; float: left; width: 200px!important;  margin: 0!important;     padding: 10px!important;}
</style>
<script>
jQuery(function(){

            spider_frontend_ajax(
			'gal_front_form_<?php echo $bwg; ?>'
			, '<?php echo $bwg; ?>'
			, 'bwg_album_compact_<?php echo $bwg; ?>'
			, jQuery(".click_0").attr("data-alb_gal_id")
			, '<?php echo $album_gallery_id; ?>'
			, jQuery(".click_0").attr("data-def_type")
			, ''
			, jQuery(".click_0").attr("data-title")
			, 'default'); 
			
			jQuery(".click_0").addClass("active");
			
        jQuery(".justinsGal a").click(function(){
            spider_frontend_ajax(
			'gal_front_form_<?php echo $bwg; ?>'
			, '<?php echo $bwg; ?>'
			, 'bwg_album_compact_<?php echo $bwg; ?>'
			...
			jQuery(".justinsGal a").removeClass("active");
...
<div class="clearfix">
	<div class="justinsGalGroup">
		<?PHP 
		if(!empty($album_galleries_row)){
			$i=0;
		...
		?>
		<div class="justinsGal " >
			<a class=" click_<?= $i; ?>  Jwg_album_<?php echo $bwg; ?>" 
			<?php echo ($from !== "widget" ? ($options_row->enable_seo ? "href='" . esc_url(add_query_arg(array("type_" . $bwg => $def_type, "album_gallery_id_" . $bwg => $album_galallery_row->alb_gal_id, "bwg_previous_album_id_" . $bwg => $album_gallery_id . ',' . $bwg_previous_album_id , "bwg_previous_album_page_number_" . $bwg => (isset($_REQUEST['page_number_' . $bwg]) ? esc_html($_REQUEST['page_number_' . $bwg]) : 0) . ',' . $bwg_previous_album_page_number), $_SERVER['REQUEST_URI'])) . "'" : "") . " 
			data-alb_gal_id=\"" . $album_galallery_row->alb_gal_id . "\" 
			data-def_type=\"" . $def_type . "\" 
			data-title=\"" . htmlspecialchars(addslashes($title)) . "\"" : "href='" . $permalink . "'") ?>>
				<?= $title ?>
			</a>
		</div>
		<?PHP $i++; } } ?>	
		...
		?>
    <div class="" id="bwg_container1_<?php echo $bwg; ?>"> 
              ...
              elseif ($type == 'gallery') {
                ...
                foreach ($image_rows as $image_row) {
                  ...
                  $thumb_top = ($params['compuct_album_image_thumb_height'] - $image_thumb_height) / 2;
					...
					-->
					<img class="bwg_img_clear bwg_img_custom"  id="<?php echo $image_row->id; ?>" src="<?php echo ( $is_embed ? "" : site_url() . '/' . $WD_BWG_UPLOAD_DIR) . $image_row->thumb_url; ?>" alt="<?php echo $image_row->alt; ?>" />		
<?PHP 
				 echo "<script>   </script>";
?>					
					<!--
